...
exports.verifyTransaction = async (req, res) => {
    try {
        ...
        res.status(200).json({ status: "Success", message: "Transaction verified", transaction: transaction });
    } catch (error) {
        res.status(500).json({ status: "Failed", message: error.message });
    }
};
...
// New createPayment function
exports.createPayment = async (req, res) => {
    try {
        const { userId, churchId, email, amount, type, projectId } = req.body;
        ...
        const newTransaction = await Transaction.create({
            ...
            status: "Pending",
            email: email,
            reference: "some-reference" // Replace with actual reference logic
        });
...
// New getPayment function
exports.getPayment = async (req, res) => {
    try {
        const paymentId = req.params.id;
        const payment = await Transaction.findById(paymentId).populate('userId').populate('churchId');
